...
jest.mock("../../features/authentication/hooks/useFetchSession", () => ({
  ...
  default: () => ({
    data: null,
    error: null,
    fetchSession: mockUseFetchSession,
  }),
}));
...
describe("Auth", () => {
  describe("AuthProvider", () => {
    ...
    it("should fetch session in the first visit", async () => {
      const children = <div>Test children</div>;
      render(<AuthProvider>{children}</AuthProvider>);

      await waitFor(() => {
        expect(mockUseFetchSession).toBeCalled();
      });
    });
      ...
      const TestComponent = () => {
        ...
        return (
          <div>
            ...
            <button
              onClick={() => logInAs({ email: "test@example.com" } as UserType)}
            >
              Log in
            </button>
